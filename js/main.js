$(function(){
    
    //============
	//! Globales
	//============
    
    sexo = {
        F : 'data:image/png;base64,',
        M : 'data:image/png;base64,'
    }
	
	//============================
	//! Validación de Passwords
	//============================
	
	$('[type=password]').keypress(function(e){

		if($(document).width()>991)
		{
			var password = $(this);
			var tooltipVisible = $('.popover').is(':visible');
			var s = String.fromCharCode(e.which);
			
			password.popover({
				container : "body",
				placement : "left",
				content : "Bloq Mayús activo"
			});
	  
			if(s.toUpperCase() === s && s.toLowerCase() !== s && !e.shiftKey) 
			{
				if (!tooltipVisible) password.popover('show');
			}
			else
			{
				if (tooltipVisible) password.popover('hide');
			}
		}
		
	});
	
	$('[type=password]').blur(function(e){
		$(this).popover('destroy');
	});
	
	$('.btn-ver-password').click(function(e){
		e.preventDefault();
		
		var password = $(e.currentTarget).parent().parent().children('input');
		
		if(password.attr('type') == 'password')
		{
			password.attr('type', 'text');
		}
		else
		{
			password.attr('type', 'password');
		}
		
		password.focus();
	});
	
	//=============
	//! Dropdowns
	//=============
	
	$('.side-nav .dropdown').each(function(i, item){
		if( ! $('li', this).length) $(this).remove();
	});
	
	$('.dropdown').on('show.bs.dropdown', function(e){
    	$(this).find('.dropdown-menu').first().stop(true, true).slideDown();
    });
  
  	$('.dropdown').on('hide.bs.dropdown', function(e){
    	$(this).find('.dropdown-menu').first().stop(true, true).slideUp();
	});
	
	//=====================
	//! Listeners botones
	//=====================
	
	$('body').on('click', '.btn-eliminar', function(){
		if( ! confirm('Desea eliminar este elemento?')) return false;
	});
	
	$('body').on('click', '.btn-cancelar', function(){
		if( ! confirm('Desea cancelar la operación actual?')) return false;
	});

	//==============================
	//! Override Bootstrap tooltip
	//==============================
	
	$('body').tooltip({
		selector: '[data-toggle=tooltip]',
		container: 'body'
	});
	
	//==============
	//! Auto focus
	//==============
    
    $('.autofocus').on('shown.bs.modal', function (e) {
		$('input', this).first().focus();
	});
    
	//================
	//! Disable Form
	//================
    
    $('form').submit(function(e){
         $('button[type=submit], .submit', e.currentTarget).addClass('disabled').empty().append(
             $('<i>', { class : 'fa fa-spin fa-cog' }), ' Cargando'
         ).blur();
    });
    
    formato_fecha = (function(fecha){
        switch(fecha)
		{
			case '01':
				return 'Ene';
			break;
  		
			case '02':
				return 'Feb';
			break;
			
			case '03':
				return 'Mar';
			break;
			
			case '04':
				return 'Abr';
			break;
			
			case '05':
				return 'May';
			break;
			
			case '06':
				return 'Jun';
			break;
			
			case '07':
				return 'Jul';
			break;
			
			case '08':
				return 'Ago';
			break;
			
			case '09':
				return 'Sep';
			break;
			
			case '10':
				return 'Oct';
			break;
			
			case '11':
				return 'Nov';
			break;
			
			case '12':
				return 'Dic';
			break;
		}   
    });
    
});
