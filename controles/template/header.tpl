...
{/if}
    ...
    <span>{$SESSION_NOMBRE}</span><br />
<span>{$SESSION_TIPO}</span>
    </div>
    </div>
    </li>
    
            <li class="{if $COM eq 'control'}active{/if}"><a href="index.php?com=control"><i class="fa fa-tasks"></i> Controles</a></li>
            <li class="{if $COM eq 'tarea'}active{/if}"><a href="index.php?com=tarea"><i class="fa fa-list"></i> Mis Controles</a></li>
            {if $SESSION_TIPO eq 'ADM'}
            <li class="dropdown {if $COM eq 'contrato' or $COM eq 'actividad' or $COM eq 'manual' or $COM eq 'privilegio' or $COM eq 'usuario'}open active{/if}">
