		...
		<nav class="navbar navbar-default navbar-fixed-top">
	        ...
	        <div class="navbar-header">
	            ...
	            <a class="navbar-brand" href="<?= site_url(''); ?>">
			        <i class="fa fa-check-square-o"></i> EVALUACIÓN EPPO
			    </a>
	        </div>
		    	...
		    	<ul class="nav navbar-nav side-nav">
			    	...
			    	<li class="nav-user hidden-xs">
			    		<div class="row">
			    			...
			    			<div class="col-sm-8 nombre">
                                ...
                                <em><?= $this->session->userdata('privilegio'); ?></em>
			    			</div>
			    		</div>
			    	</li>
			    	
	            	<li class="<?= ((!$this->uri->segment(1)) OR ($this->uri->segment(1) == 'inicio') OR ($this->uri->segment(1) == 'formulario')) ? 'active': ''; ?>"><a href="<?= site_url(''); ?>"><i class="fa fa-home"></i> Proceso Periodo actual</a></li>
                    
                    <li class="<?= ($this->uri->segment(1) == 'historial') ? 'active': ''; ?>"><a href="<?= site_url('historial'); ?>"><i class="fa fa-history"></i> Mi historial</a></li>
                    
                    <?php if($this->session->userdata('evaluador')){ ?>
                    <li class="<?= ($this->uri->segment(1) == 'evaluador') ? 'active': ''; ?>"><a href="<?= site_url('evaluador'); ?>"><i class="fa fa-users"></i> Mis evaluados</a></li>
                    <?php } ?>
                    
                    <?php if(($this->session->userdata('privilegio') == 'ADM') OR ($this->session->userdata('privilegio') == 'COR')){ ?>
                    <li class="<?= ($this->uri->segment(1) == 'coordinacion') ? 'active': ''; ?>"><a href="<?= site_url('coordinacion'); ?>"><i class="fa fa-users"></i> Coordinación</a></li>
                    <?php } ?>
                    
                    <?php if($this->session->userdata('comite') OR ($this->session->userdata('privilegio') == 'ADM')){ ?>
                    <li class="<?= ($this->uri->segment(1) == 'comite') ? 'active': ''; ?>"><a href="<?= site_url('comite'); ?>"><i class="fa fa-users"></i> Comité evaluador</a></li>
                    <?php } ?>
	            	...
	            	<li class="dropdown <?= ($this->uri->segment(1) == 'admin') ? 'active open': ''; ?>">
		            	...
		            	<ul class="dropdown-menu">
                            <li><a href="<?= site_url('admin/periodos'); ?>">Periodo</a></li>
			            	<li><a href="<?= site_url('admin/etapas'); ?>">Etapas</a></li>
			            	<li><a href="<?= site_url('admin/comites'); ?>">Comité evaluador</a></li>
                            <li><a href="<?= site_url('admin/evaluadores'); ?>">Evaluadores</a></li>
                            <li><a href="<?= site_url('admin/formularios'); ?>">Formularios</a></li>
