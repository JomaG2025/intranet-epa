<!DOCTYPE html>
<html lang="es">
<head>
    <title>Inventario de Activos - Intranet EPA</title>
    <meta http-equiv="Content-type" content="text/html; charset=utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/bootstrap/bootstrap.min.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/bootstrap/puertoarica.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/fontawesome/font-awesome.min.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/jqueryui/jquery-ui.min.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/jqueryui/jquery-ui-timepicker-addon.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/libs/animate/animate.css">
    <link rel="stylesheet" href="https://puertoarica.cl/graficas/css/plugins/datatables/datatables.bootstrap.css">
    <link rel="stylesheet" href="<?= base_url(); ?>css/estilo.css">

	<script src="https://puertoarica.cl/graficas/js/libs/jquery/jquery.min.js"></script>
	<script src="https://puertoarica.cl/graficas/js/libs/bootstrap/bootstrap.min.js"></script>
	<script src="https://puertoarica.cl/graficas/js/libs/jqueryui/jquery-ui.min.js"></script>
    <script src="https://puertoarica.cl/graficas/js/libs/jqueryui/jquery.ui.datepicker-es.js"></script>
    <script src="https://puertoarica.cl/graficas/js/libs/jqueryui/jquery-ui-timepicker-addon.js"></script>
    <script src="https://puertoarica.cl/graficas/js/plugins/datatables/jquery.dataTables.js"></script>
    <script src="https://puertoarica.cl/graficas/js/plugins/datatables/dataTables.bootstrap.js"></script>
    <script src="https://puertoarica.cl/graficas/js/plugins/datatables/dataTables.spanish.lang.js"></script>
    <script src="https://puertoarica.cl/graficas/js/plugins/bootstrap-notify/bootstrap-notify.min.js"></script>
    <script src="<?= base_url(); ?>js/main.js"></script>
</head>

<body>

    <?php $this->load->view('alertas'); ?>

	<div id="wrapper">

		<nav class="navbar navbar-default navbar-fixed-top">

	        <div class="navbar-header">
	        	<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-ex1-collapse">
	            	<span class="sr-only">Navegación</span>
	            	<i class="fa fa-bars"></i>
	            </button>
	            <a class="navbar-brand" href="<?= site_url('inventario'); ?>">
			        <i class="fa fa-cubes"></i> Inventarios
			    </a>
	        </div>

	        <div class="collapse navbar-collapse navbar-ex1-collapse">

		    	<ul class="nav navbar-nav side-nav">

			    	<li class="nav-user hidden-xs">
			    		<div class="row">
				    		<div class="col-sm-4">
					    		<?php if($this->session->userdata('sexo') == 'F'){ ?>
			    				<img src="data:image/png;base64," class="img-responsive img-circle" alt=""/>
								<?php }else{ ?>
			    				<img src="data:image/png;base64," class="img-responsive img-circle" alt=""/>
								<?php } ?>
			    			</div>
			    			<div class="col-sm-8 nombre">
			    				<strong><?= $this->session->userdata('nombre'); ?></strong><br />
                                <em><?php
                                    // El privilegio mostrado aca es el PROPIO de Inventario
                                    // (inv_usuarios), no el privilegio general de la intranet -- son
                                    // cosas distintas a proposito. Ver
                                    // application/helpers/inv_permisos_helper.php.
                                    $inv_usuario_sesion = inv_usuario_actual();
                                    echo $inv_usuario_sesion ? inv_e($inv_usuario_sesion['privilegio']) : '';
                                ?></em>
			    			</div>
			    		</div>
			    	</li>

		            <?php
		                // Permisos propios del modulo (tabla inv_usuarios) -- YA NO el
		                // privilegio general de la intranet. Ver
		                // application/helpers/inv_permisos_helper.php.
		                $inv_permiso_acceso = inv_tiene_acceso();
		                $inv_permiso_editar = inv_puede_editar();
		                $inv_permiso_admin  = inv_es_admin();

		                // Resaltado del item activo del menu: se usa el controlador/metodo
		                // REAL (post-enrutamiento), no $this->uri->segment(), porque las
		                // rutas amigables definidas en parches/05_rutas_inventario.md (ej.
		                // /inventario/importar, /inventario/admin/usuarios) hacen que la
		                // URI visible y el controlador real difieran.
		                $inv_clase = $this->router->fetch_class();
		                $inv_metodo = $this->router->fetch_method();

		                $inv_resaltar_dashboard       = ($inv_clase == 'inventario' AND $inv_metodo == 'index');
		                $inv_resaltar_nuevo           = ($inv_clase == 'inventario' AND in_array($inv_metodo, array('nuevo', 'grabar')));
		                $inv_resaltar_activos         = ($inv_clase == 'inventario' AND ( ! $inv_resaltar_dashboard) AND ( ! $inv_resaltar_nuevo));
		                $inv_resaltar_importar        = ($inv_clase == 'inventario_importacion' AND $inv_metodo != 'historial');
		                $inv_resaltar_importaciones   = ($inv_clase == 'inventario_importacion' AND $inv_metodo == 'historial');
		                $inv_resaltar_admin_catalogos = ($inv_clase == 'inventario_catalogos');
		                $inv_resaltar_admin_usuarios  = ($inv_clase == 'inventario_usuarios');
		                $inv_resaltar_admin           = ($inv_resaltar_admin_catalogos OR $inv_resaltar_admin_usuarios);
		            ?>

		            <?php if ($inv_permiso_acceso) { ?>

		            <li class="<?= $inv_resaltar_dashboard ? 'active' : ''; ?>"><a href="<?= site_url('inventario'); ?>"><i class="fa fa-tachometer"></i> Dashboard</a></li>
		            <li class="<?= $inv_resaltar_activos ? 'active' : ''; ?>"><a href="<?= site_url('inventario/listado'); ?>"><i class="fa fa-list"></i> Activos</a></li>

		            <?php if ($inv_permiso_editar) { ?>
		            <li class="<?= $inv_resaltar_nuevo ? 'active' : ''; ?>"><a href="<?= site_url('inventario/nuevo'); ?>"><i class="fa fa-plus"></i> Nuevo Activo</a></li>
		            <?php } ?>

		            <?php if ($inv_permiso_admin) { ?>
		            <li class="<?= $inv_resaltar_importar ? 'active' : ''; ?>"><a href="<?= site_url('inventario/importar'); ?>"><i class="fa fa-upload"></i> Importar Excel</a></li>
		            <li class="<?= $inv_resaltar_importaciones ? 'active' : ''; ?>"><a href="<?= site_url('inventario/importaciones'); ?>"><i class="fa fa-history"></i> Historial de Importaciones</a></li>

		            <li class="dropdown <?= $inv_resaltar_admin ? 'active open' : ''; ?>">
		            	<a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-cogs"></i> Administración &nbsp;<i class="fa fa-caret-down"></i></a>
		            	<ul class="dropdown-menu">
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/gerencias'); ?>">Gerencias</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/areas'); ?>">Áreas</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/ubicaciones'); ?>">Ubicaciones</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/categorias'); ?>">Categorías</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/subcategorias'); ?>">Subcategorías</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/tipos'); ?>">Tipos</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/responsables'); ?>">Responsables</a></li>
			            	<li><a href="<?= site_url('admin/inventario_catalogos/index/estados'); ?>">Estados</a></li>
			            	<li class="divider"></li>
			            	<li><a href="<?= site_url('inventario/admin/usuarios'); ?>">Usuarios</a></li>
			            </ul>
				        </li>
				        <?php } ?>

				        <?php } // fin if ($inv_permiso_acceso) ?>

			        <li><a href="<?= site_url(); ?>"><i class="fa fa-reply"></i> Volver a Inicio</a></li>
			        <li><a href="<?= site_url('login/salir'); ?>"><i class="fa fa-power-off"></i> Salir</a></li>

			    </ul>

			</div>
		</nav>

		<div id="page-wrapper">
