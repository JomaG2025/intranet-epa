$(function(){
	...
	$('.side-nav .dropdown').each(function(i, item){
    ...
    $('body').on('click', '.btn-convertir-privado', function(){
		if( ! confirm('Desea hacer privado este elemento?')) return false;
	});
    
    $('body').on('click', '.btn-convertir-publico', function(){
		if( ! confirm('Desea hacer público este elemento?')) return false;
	});
	
	$('body').on('click', '.btn-cancelar', function(){
		if( ! confirm('Desea cancelar la operación actual?')) return false;
	});
    ...
    $('.autofocus').on('shown.bs.modal', function (e) {
    ...
    $('form').submit(function(e){
         $('button[type=submit], .submit', e.currentTarget).addClass('disabled').empty().append(
             $('<i>', { class : 'fa fa-spin fa-cog' }), ' Cargando'
         ).blur();
    });
});
